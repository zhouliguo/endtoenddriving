...
class ResNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.input_n != 0:
            output = self.catchImageWithPosImagePast(image, images_past)
        else:
            output = image

        return output, normalized_coordinates, target
    
    def catchImageWithPosImagePast(self, images, image_past):
        '''
        增加图片通道，将当前图片和过去input-n个图片拼接起来
        '''

        num_channels, height, width = images.shape
        num_channels_past, height, width = image_past.shape
        combine_channel = np.zeros((num_channels + num_channels_past, height, width), 
                                 dtype=images.dtype)
        combine_channel[:num_channels, :, :] = images
        combine_channel[num_channels:num_channels + num_channels_past, :, :] = image_past
...
if __name__=='__main__':
    ...
    for i, (output, coor, target) in enumerate(train_dataloader):
        if i == 3:
            print(i, output.shape) # torch.Size([16, 6, 384, 704])
