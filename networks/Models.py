...
class FDN(nn.Module):
    def __init__(self, out_features):
        ...
        self.linear2 = nn.Linear(32, out_features)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        if mask is not None:
            ...
            # 原始张量的形状是 (3, 4)，使用 unsqueeze(1) 操作后，形状将变为 (3, 1, 4)
...
class PositionalEncoding(nn.Module):
    ...
    def _get_sinusoid_encoding_table(self, n_position, d_hid):
        ...
        def get_position_angle_vec(position):
            return [(256*position/28) / np.power(10000, 2 * (hid_j // 2) / d_hid) for hid_j in range(d_hid)]
...
class Decoder(nn.Module):
    ...
    def __init__(
            ...
            d_model, d_inner, pad_idx, n_position_d=200, dropout=0.1, scale_emb=False):

        super().__init__()

        self.trg_word_emb = nn.Embedding(n_trg_vocab, d_word_vec, padding_idx=pad_idx)
        self.position_enc = PositionalEncoding(d_word_vec, n_position=n_position_d)
        self.dropout = nn.Dropout(p=dropout)
        self.layer_stack = nn.ModuleList([
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.d_model = d_model
        self.fpn1 = FPN(input_dim, 256)
        self.fpn2 = FPN(output_dim, 256)
        ...
        self.decoder = Decoder(
            n_trg_vocab=n_trg_vocab, n_position_d=n_position_d,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=n_layers,pad_idx=1, n_head=n_head, d_k=d_k, d_v=d_v,
           dropout=dropout)

        self.fdn = FDN(output_dim)
    ...
    def forward(self, input_data, output_data):
        input_data = self.fpn1(input_data)
        input_data = self.conv(input_data.permute(0, 2, 1)).permute(0, 2, 1)
        output_data = self.fpn2(output_data)
        src_seq = input_data.to(torch.float32)
        trg_seq = output_data.to(torch.float32)
